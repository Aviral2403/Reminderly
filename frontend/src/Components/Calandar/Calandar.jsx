...
const Calandar = ({ user }) => {
    ...
    if (user?._id) {
      try {
        const response = await axios.get(
          `https://reminderly-backend-hub.onrender.com/api/events/user/${user._id}/upcoming`
        );
  ...
  const handleAddEvent = async () => {
    ...
    try {
      const response = await axios.post(
        `https://reminderly-backend-hub.onrender.com/api/events/create`,
        newEvent
      );
  ...
  const handleDeleteEvent = async (eventId) => {
    if (window.confirm("Are you sure you want to delete this event?")) {
      try {
        await axios.delete(`https://reminderly-backend-hub.onrender.com/api/events/${eventId}`);
