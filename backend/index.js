...
const resend = new Resend("re_UPEGyzpB_96T4dTFZDKL75WQPtWp3xX6A"); // Replace with your actual Resend API key
...
const getEvents = async (req, res) => {
  ...
  const eventsWithOwner = await Promise.all(events.map(async event => {
    try {
      ...
      // Add a null check for owner
      if (!owner) {
        console.warn(`No user found for event ID: ${event._id} with userId: ${event.userId}`);
        return {
          ...event._doc,
          owner: null // or a default email
        };
      }
      ...
      console.error(`Error finding owner for event ${event._id}:`, error);
      return {
        ...event._doc,
        owner: null
      };
